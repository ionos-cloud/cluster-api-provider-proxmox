...
func TestReconcileIPAddresses_CreateAdditionalClaim(t *testing.T) {
	...
	extraPool0 := corev1.TypedLocalObjectReference{APIGroup: new("ipam.cluster.x-k8s.io"),
...
func TestReconcileIPAddresses_DuplicatePoolRefOffsetsAppendAddresses(t *testing.T) {
	...
	createIPPools(t, kubeClient, machineScope)
	createIPAddress(t, kubeClient, machineScope, infrav1.DefaultNetworkDevice, "192.0.2.10", 0, &duplicatePool)
	createIPAddress(t, kubeClient, machineScope, infrav1.DefaultNetworkDevice, "192.0.2.11", 1, &duplicatePool)
...
func TestReconcileIPAddresses_TwoDistinctPoolsOnOneInterface(t *testing.T) {
	machineScope, _, kubeClient := setupReconcilerTestWithCondition(t, infrav1.ProxmoxMachineVirtualMachineProvisionedWaitingForStaticIPAllocationReason)

	poolA := corev1.TypedLocalObjectReference{
		APIGroup: new("ipam.cluster.x-k8s.io"),
		Kind:     GlobalInClusterIPPool,
		Name:     "pool-a",
	}
	poolB := corev1.TypedLocalObjectReference{
		APIGroup: new("ipam.cluster.x-k8s.io"),
		Kind:     GlobalInClusterIPPool,
		Name:     "pool-b",
	}
	...
	machineScope.ProxmoxMachine.Spec.Network = &infrav1.NetworkSpec{
		NetworkDevices: []infrav1.NetworkDevice{{
			Name: infrav1.DefaultNetworkDevice,
			InterfaceConfig: infrav1.InterfaceConfig{
				IPPoolRef: []corev1.TypedLocalObjectReference{poolA, poolB},
			},
		}},
	}
	...
	createIPPools(t, kubeClient, machineScope)
	createIPAddress(t, kubeClient, machineScope, infrav1.DefaultNetworkDevice, "192.0.2.10", 0, &poolA)
	createIPAddress(t, kubeClient, machineScope, infrav1.DefaultNetworkDevice, "192.0.2.11", 1, &poolB)
...
func TestReconcileIPAddresses_MultipleDevices(t *testing.T) {
	...
	ipv4pool0 := corev1.TypedLocalObjectReference{APIGroup: new("ipam.cluster.x-k8s.io"),
		Kind: GlobalInClusterIPPool,
		Name: "ipv4pool0",
	}
	ipv4pool1 := corev1.TypedLocalObjectReference{APIGroup: new("ipam.cluster.x-k8s.io"),
		Kind: GlobalInClusterIPPool,
		Name: "ipv4pool1",
	}
	ipv6pool := corev1.TypedLocalObjectReference{APIGroup: new("ipam.cluster.x-k8s.io"),
		Kind: GlobalInClusterIPPool,
		Name: "ipv6pool",
	}
...
func TestReconcileIPAddresses_IPv6(t *testing.T) {
	...
	defaultPoolV6 := corev1.TypedLocalObjectReference{APIGroup: new("ipam.cluster.x-k8s.io"),
...
func TestReconcileIPAddresses_DefaultMapIgnoresNonDefaultPoolWhenIPv6Default(t *testing.T) {
	...
	vm := newStoppedVM()
	vm.VirtualMachineConfig.Tags = "ip_net0_192.0.2.10"
	machineScope.SetVirtualMachine(vm)

	createIPPools(t, kubeClient, machineScope)
	createIPAddress(t, kubeClient, machineScope, infrav1.DefaultNetworkDevice, "192.0.2.10", 0, &defaultPool)
	createIPAddress(t, kubeClient, machineScope, infrav1.DefaultNetworkDevice, "2001:db8::1", 1, &defaultPoolV6)
	...
	require.Equal(t,
		infrav1.IPAddressesSpec{
			...
			IPv6:    []string{"2001:db8::1", "2001:db8::10"},
		},
		*machineScope.ProxmoxMachine.GetIPAddressesNet(infrav1.DefaultNetworkDevice),
	)
	require.Equal(t,
...
func TestReconcileIPAddresses_MachineIPPoolRef(t *testing.T) {
	...
	extraPool1 := corev1.TypedLocalObjectReference{APIGroup: new("ipam.cluster.x-k8s.io"),
		Kind: GlobalInClusterIPPool,
		Name: "extrapool1",
	}
	extraPool2 := corev1.TypedLocalObjectReference{APIGroup: new("ipam.cluster.x-k8s.io"),
		Kind: GlobalInClusterIPPool,
		Name: "extrapool2",
	}
