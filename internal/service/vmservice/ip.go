...
func handleDefaultDevice(ctx context.Context, machineScope *scope.MachineScope, addresses map[string]infrav1alpha2.IPAddress) (bool, error) {
	...
	if machineScope.InfraCluster.ProxmoxCluster.Spec.IPv4Config != nil {
		ip, err := handleIPAddressForDevice(ctx, machineScope, infrav1alpha2.DefaultNetworkDevice, infrav1alpha2.IPV4Format, nil)
		if err != nil || ip == "" {
			return true, err
		}
	...
	if machineScope.InfraCluster.ProxmoxCluster.Spec.IPv6Config != nil {
		ip, err := handleIPAddressForDevice(ctx, machineScope, infrav1alpha2.DefaultNetworkDevice, infrav1alpha2.IPV6Format, nil)
		if err != nil || ip == "" {
			return true, err
		}
