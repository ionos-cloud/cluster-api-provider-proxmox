...
if [[ "${DOCKERFILE_GO}" != "${GO_MINOR}" ]]; then
...
if [[ "${CAPI_REQ}" != "${CAPI_REP}" ]]; then
    CAPI_NEEDS_BUMP=true
fi
if [[ "${CAPI_REQ}" != "${CAPI_TEST}" ]]; then
    CAPI_NEEDS_BUMP=true
fi

if [[ "${CAPI_NEEDS_BUMP}" == true ]]; then
    echo "Auto-bump: cluster-api ${CAPI_REQ}"
    "${SCRIPT_DIR}/bump-capi.sh" "${CAPI_REQ}"
fi
...
if [[ -n "${K8S_LATEST}" ]]; then
    ...
    for pkg in 'k8s.io/api' 'k8s.io/apimachinery' 'k8s.io/client-go'; do
        ...
        if [[ "${req}" != "${K8S_LATEST}" ]]; then
            NEEDS_BUMP=true
            break
        fi
        rep=$(gomod_get_replace "${pkg}")
        if [[ -n "${rep}" && "${rep}" != "${K8S_LATEST}" ]]; then
            NEEDS_BUMP=true
            break
        fi
    done
