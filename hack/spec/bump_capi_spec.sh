Describe 'bump-capi.sh'
  ...
  It 'bumps cluster-api version'
    When run script ../bump-capi.sh 1.11.0
  ...
  It 'updates go.mod require version'
    bash ../bump-capi.sh 1.11.0 >/dev/null 2>&1
  ...
  It 'does not duplicate existing metadata entry'
    When run script ../bump-capi.sh 1.10.5
    The status should be success
  ...
  It 'fails with invalid version'
    When run script ../bump-capi.sh 'bad'
    The status should be failure
    The error should include 'invalid version format'
  End

  It 'registers the release in every e2e contract catalog'
    bash ../bump-capi.sh 1.11.0 >/dev/null 2>&1
