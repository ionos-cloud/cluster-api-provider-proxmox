...
func (e *Enricher) getProxmoxEnvContent() string {
	...
	_, _ = content.WriteString("COREOS_CUSTOM_HOSTNAME=" + e.Hostname + "\nCOREOS_CUSTOM_INSTANCE_ID=" + e.InstanceID + "\nCOREOS_CUSTOM_PROVIDER_ID=" + e.ProviderID)
	...
	for _, network := range e.Network {
		for _, ipconfig := range network.IPConfigs {
			if ipconfig.IPAddress.Addr().Is4() && ipconfig.Default {
				_, _ = content.WriteString("\nCOREOS_CUSTOM_PRIVATE_IPV4=" + ipconfig.IPAddress.String())
			}
			if ipconfig.IPAddress.Addr().Is6() && ipconfig.Default {
				_, _ = content.WriteString("\nCOREOS_CUSTOM_PRIVATE_IPV6=" + ipconfig.IPAddress.String())
			}
		}
	}
